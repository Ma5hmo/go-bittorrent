...
import PeerDB from './database.js'
...
class Swarm {
  constructor (infoHash, server) {
    ...
    self.peerDB = Swarm.peerDBInstance || (Swarm.peerDBInstance = new PeerDB())

    // Load peers from DB on startup
    self.peerDB.loadPeers(infoHash, (err, rows) => {
      if (!err && Array.isArray(rows)) {
        rows.forEach(row => {
          self.peers.set(row.peer_id, {
            type: 'http', // only http/udp supported
          ...
          if (row.complete) self.complete++
          else self.incomplete++
        })
      }
    })
  ...
  _onAnnounceStarted (params, peer, id) {
    ...
    this.peerDB.upsertPeer(this.infoHash, newPeer)
  }
  ...
  _onAnnounceCompleted (params, peer, id) {
    ...
    this.peerDB.upsertPeer(this.infoHash, peer)
  }
